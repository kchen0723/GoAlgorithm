...
func GetCombationFromDuplicateArray(nums []int, length int) [][]int {
	...
	sort.Ints(nums)
	var result [][]int
	getCombationFromDuplicateArrayHelp(nums, length, 0, []int{}, &result)
	return result
}

func getCombationFromDuplicateArrayHelp(nums []int, length int, index int, candidate []int, result *[][]int) {
	if len(candidate) == length {
		item := append([]int(nil), candidate...)
	...
	for i := index; i < len(nums); i++ {
		...
		getCombationFromDuplicateArrayHelp(nums, length, i+1, candidate, result)
